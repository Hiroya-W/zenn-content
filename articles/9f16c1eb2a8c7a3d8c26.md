...
Zenn CLIとtextlint関連のパッケージをインストールし、`package.json`と`package-lock.json`にパッケージ情報を追加します。
...
`articles`や`books`などのZenn用のフォルダやファイルを生成しておきます。
...
先程作成した`package.json`や`package-lock.json`を用いてコンテナを作成できるようにし、Zenn用のディレクトリをマウントして利用できるようにします。

### 前準備

`package.json`と`package-lock.json`を`Dockerfile`から扱えるように、ファイルの配置位置を変更しておきます。
